...
const IMPORT_PATHS: Record<ImportKind, string[]> = {
  ...
  // config.json is the legacy Codex adapter format; config.toml is current.
  // Codex layers (legacy JSON, user TOML, project TOML) all merge, so
  // resolveImportCandidates owns their precedence ordering — do not reorder
  // expecting first-match-wins here.
  codex: [
...
export function findAvailableImportConfigs(cwd = process.cwd()): DiscoveredImportConfig[] {
  ...
  for (const importKind of Object.keys(IMPORT_PATHS) as ImportKind[]) {
    // Parse first: malformed files surface the discovery warning here and a
    // kind with no loadable file stays hidden, matching upstream behaviour.
    const imported = loadImportedConfig(importKind, cwd, `Failed to discover imported MCP config from ${importKind}:`);
    if (!imported) continue;
    // Then list every existing candidate path, not just the winning one:
    // merged kinds (Codex user/project TOML, OpenCode layers) have several
    // live files, and discovery output should show everything that is read.
...
export function loadMcpConfig(overridePath?: string, cwd = process.cwd()): McpConfig {
  ...
  for (const source of sourceSpecs) {
    const loaded = readValidatedConfig(source.readPath, `MCP config from ${source.readPath}`);
    if (!loaded) continue;
    config = mergeMcpConfigs(config, expandImports(loaded, cwd));
  }
...
function mergeServerMaps(
  ...
  for (const [name, definition] of Object.entries(next)) {
    ...
    } else if (existing?.url && typeof definition.command === "string") {
      baseEntry = { ...existing };
      for (const field of ["url", "headers", "auth", "bearerToken", "bearerTokenEnv", "oauth"] as const) {
        delete baseEntry[field];
      }
    } else if (existing?.command && typeof definition.url === "string") {
      baseEntry = { ...existing };
      for (const field of ["command", "args", "env", "cwd"] as const) {
        delete baseEntry[field];
      }
    }
...
function expandImports(config: McpConfig, cwd = process.cwd()): McpConfig {
  ...
  let importedServers: Record<string, ServerEntry> = {};
  for (const importKind of config.imports) {
    const imported = loadImportedConfig(importKind, cwd, `Failed to import MCP config from ${importKind}:`);
    if (!imported) continue;

    // Preserve the published import-order contract: later import kinds can
    // refine earlier definitions. Use the URL-aware merge so changing an
    // endpoint never carries the previous endpoint's credentials with it.
    importedServers = mergeServerMaps(importedServers, extractServers(imported.value, importKind));
  }
...
function resolveImportCandidates(importKind: ImportKind, cwd: string): string[] {
  if (importKind === "codex") {
    // Codex layers merge rather than first-match-wins, so candidates are
    // ordered from lowest to highest precedence: the legacy JSON fallback,
    // the current user TOML, then the project-local TOML discovered from cwd.
    return [...new Set([
      join(homedir(), ".codex", "config.json"),
      join(homedir(), ".codex", "config.toml"),
...
function loadImportedConfig(
  ...
  if (importKind === "codex") {
    // All existing Codex layers merge; later (higher-precedence) layers
    // override earlier ones per field, mirroring mergeServerMaps semantics.
    // A malformed layer warns and is skipped without sinking the others.
    ...
    for (const path of resolveImportCandidates(importKind, cwd)) {
      ...
      try {
        ...
        // Normalize each layer before merging so endpoint changes receive the
        // same URL-bound credential stripping as ordinary config sources.
        mergedServers = mergeServerMaps(mergedServers, extractServers(value, "codex"));
    ...
    return highestPrecedencePath ? { path: highestPrecedencePath, value: { mcpServers: mergedServers } } : null;
  }
...
function extractServers(config: unknown, kind: ImportKind): Record<string, ServerEntry> {
  ...
  for (const [name, entry] of Object.entries(servers)) {
    ...
    // Codex TOML semantics: `enabled = false` disables the server, and an
    // entry without a command or url is not runnable.
    if (rawEntry.enabled === false) continue;
    if (typeof rawEntry.command !== "string" && typeof rawEntry.url !== "string") continue;

    const mapped = { ...rawEntry };
    ...
    // Codex `env_vars` forwards process environment into the server env. An
    // explicitly declared `env` value always wins over a forwarded name.
    const envVars = stringArray(mapped.env_vars);
    if (envVars) {
      const env = { ...(mapped.env as Record<string, string> | undefined) };
      for (const envName of envVars) {
        if (/^[A-Za-z_][A-Za-z0-9_]*$/.test(envName) && env[envName] === undefined) {
          env[envName] = `$env:${envName}`;
        }
      }
      if (Object.keys(env).length > 0) mapped.env = env;
    }

    const toolTimeoutSeconds = mapped.tool_timeout_sec;
    if (
      typeof toolTimeoutSeconds === "number" && Number.isFinite(toolTimeoutSeconds) && toolTimeoutSeconds > 0
      && mapped.requestTimeoutMs === undefined
    ) {
      mapped.requestTimeoutMs = toolTimeoutSeconds * 1000;
    }

    const disabledTools = stringArray(mapped.disabled_tools);
    if (disabledTools && mapped.excludeTools === undefined) {
      mapped.excludeTools = disabledTools;
    }

    delete mapped.bearer_token_env_var;
    delete mapped.http_headers;
    delete mapped.env_http_headers;
    delete mapped.env_vars;
    delete mapped.tool_timeout_sec;
    delete mapped.disabled_tools;
...
export function writeProjectServerDisabledOverride(
  ...
  if (disabled) {
    ...
    for (const source of getConfigSources(overridePath, cwd)) {
      ...
      const loaded = readValidatedConfig(source.readPath, `MCP config from ${source.readPath}`);
      if (loaded) lowerConfig = mergeMcpConfigs(lowerConfig, expandImports(loaded, cwd));
    }
    if (raw.imports !== undefined) {
      ...
      lowerConfig = mergeMcpConfigs(lowerConfig, expandImports({ mcpServers: {}, imports: raw.imports as ImportKind[] }, cwd));
    }
