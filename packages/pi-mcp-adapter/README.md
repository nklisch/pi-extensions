...
- In proxy and programmatic mode, `details.mcpResult` is kept raw when its JSON is **≤ 16 KiB**; larger results are replaced with a compact summary and the complete result is saved to a temp file as readable JSON. When text and details overflow in the same call, that full-result file is the single recovery artifact — the truncation notice points at it instead of writing a second text spill. Direct tools keep their lean details and never carry `mcpResult`.
