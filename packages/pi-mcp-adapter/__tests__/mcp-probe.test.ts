...
function mockFetch(...responses: Response[]): void {
  fetchMock.mockImplementation(async () => responses[responses.length - 1]!.clone());
  fetchMock.mockResolvedValueOnce(responses[0].clone());
  for (const response of responses.slice(1)) fetchMock.mockResolvedValueOnce(response.clone());
...
describe("MCP endpoint shape probe", () => {
  ...
  it("retries legacy initialization after modern discovery is rejected", async () => {
    mockFetch(
    ...
    expect(fetchMock.mock.calls[1]?.[1]).toMatchObject({ method: "POST", headers: { Accept: "application/json, text/event-stream" } });
