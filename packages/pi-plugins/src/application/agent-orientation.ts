...
function inventoryHeader(input: AgentOrientationInput, plugins: readonly OrientationPlugin[]): string {
  return [
    `<!-- Generated by pi-plugin-host ${text(input.packageVersion) || "unknown"} at session start and after plugin changes. Do not edit. -->`,
...
function injectionLines(input: AgentOrientationInput, plugins: readonly OrientationPlugin[]): readonly string[] {
  const enabled = plugins.filter((plugin) => plugin.activation !== "disabled");
  const visible = enabled.slice(0, 6);
  const more = enabled.length - visible.length;
