...
const definitions = [
  ...
  { key: "degraded", plugin: "degraded@market", subject: "installed", finding: "pluginDegraded", activation: "enabled", update: "current", trust: "authorized" },
...
export const SplitInspectorFixture = Object.freeze(definitions.map((definition, index) => {
  ...
  const activation = !installed ? undefined : {
    ...
    state: definition.finding === "pluginDegraded" ? "degraded" as const : definition.activation === "disabled" ? "inactive" as const : definition.finding === "projectUntrusted" ? "blocked" as const : "active" as const,
  ...
  const detail = NativeInspectionDetailSchema.parse({
    ...
    configuration: [],
    lifecycle: { installed, ...(installed ? { activationIntent: definition.activation } : {}), health: definition.finding === "pluginDegraded" ? "degraded" as const : "none" as const, update: definition.update },
