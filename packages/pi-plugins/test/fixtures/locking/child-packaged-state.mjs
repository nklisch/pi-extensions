...
const currentProject = createScopeContext({ kind: "project", identity, projectKey: deriveProjectKey(identity, sha256) }, sha256);
const adapters = await createNodeLifecycleStateAdapters({ paths: createPluginHostPathPlan(agentDir), currentProject, sha256, verifyLocalFilesystem: async () => {} });
try {
  if (mode === "reader") {
    let previous = 0;
    for (let index = 0; index < count; index += 1) {
      const loaded = await adapters.state.read({ kind: "user" }, new AbortController().signal);
      if (!loaded.ok || loaded.snapshot.generation < previous || loaded.snapshot.config.generation !== loaded.snapshot.generation || loaded.snapshot.installed.generation !== loaded.snapshot.generation || loaded.snapshot.trust.generation !== loaded.snapshot.generation) throw new Error("reader observed a partial lifecycle generation");
    ...
    while (committed < count) {
      ...
      const mutation = parseStateMutation({ scope: { kind: "user" }, expectedGeneration: loaded.snapshot.generation, replace: { config: { schemaVersion: 4, generation: loaded.snapshot.generation, records: [] } } }, sha256);
    ...
    const mutation = parseStateMutation({ scope: { kind: "user" }, expectedGeneration: 0, replace: { config: { schemaVersion: 4, generation: 0, records: [] } } }, sha256);
    const result = await adapters.state.commit(mutation, new AbortController().signal);
