...
function snapshot(epoch: string) {
  return { binding: { capturedAt: 1, scopes: [{ scope: { kind: "user" }, generation: 0, status: "ready", corruptionCodes: [] }], currentProject: { projectKey: `project-v1:sha256:${"11".repeat(32)}`, trust: { kind: "trusted" }, epoch: `sha256:${"22".repeat(32)}` }, catalogs: [], capability: { status: "ready", digest: `sha256:${"33".repeat(32)}`, capturedBy: "fixture" }, runtimeEpoch: `sha256:${epoch.repeat(64).slice(0, 64)}`, convergenceDigest: `sha256:${"44".repeat(32)}`, updateDigest: `sha256:${"55".repeat(32)}` }, states: [], currentProject: { identity: { kind: "path-only", canonicalRoot: "file:///project/", limitation: "identity-changes-with-canonical-root" }, projectKey: `project-v1:sha256:${"11".repeat(32)}`, trust: { kind: "trusted" } }, runtime: [], recovery: { results: [], deferred: false, processed: 0 }, startup: { status: "ready", blocked: [], capabilities: { mcp: { status: "unavailable", explanation: "none" }, subagents: { status: "unavailable", explanation: "none" }, piReload: { status: "available", explanation: "ready" }, secrets: { status: "available", explanation: "ready" } } } } as never;
}
