...
User-owned global/project config may widen policy. Repository policy and installed packs cannot silently widen it. Persisted settings contain only non-default choices, while runtime normalization supplies the rest. Package installation may repair existing materialized config into that sparse form, with backups and no creation of absent files; it never enables packs. Symlinked config files and project directories are reported and left untouched rather than followed. Tune replay, adversarial checks, explicit confirmation, and post-write validation make policy growth auditable. The default non-Bash bypass is an intentional published behavioral break requiring next-minor release communication.
