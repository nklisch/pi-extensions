...
PACKAGE_NAME="$(node -e 'const fs=require("node:fs"); console.log(JSON.parse(fs.readFileSync(process.argv[1], "utf8")).name)' "$PKG_DIR/package.json")"
WORK="$(mktemp -d)"
...
import { loadLayeredSettings, type LayeredSettingsSource } from "$PACKAGE_NAME/settings";
